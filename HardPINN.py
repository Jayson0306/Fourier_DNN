...
class MscaleDNN(tn.Module):
    ...
    def loss_AH(self, XY=None, D_xy=None, if_lambda2Ubd=True, loss_type='l2_loss', scale2lncosh=0.5,
                rho=1.239, Cp=1.005, k1=0.024, T_in=30, R1=0.05, q=1, u_max=0.02):
        ...
        h_xy = T_in + torch.mul(Distance, UNN)
        ...
        grad2DNN = torch.autograd.grad(Distance, X, grad_outputs=torch.ones_like(X), create_graph=True, retain_graph=True)
        Distance2x = grad2DNN[0].reshape(-1, 1)
        u= torch.mul(u_max, torch.sub(torch.ones_like(Y), torch.square(Y/R1)))
        res1 = rho * Cp * torch.mul(u, (torch.mul(Distance2x, UNN)+torch.mul(Distance, dUNN2x)))
        res2= k1 * (torch.mul(Distance, dUNNxx)+ torch.mul(Distance2x, dUNN2x) + torch.mul(Distance, dUNNyy))
        res3=q
        res = res1 - res2 - res3
    ...
    def loss2bd_neumann_AH(self, XY_bd=None, Ubd_exact=None, T_in=30, L=1,
        ...
        grad2UNN = torch.autograd.grad(UNN, XY_bd, grad_outputs=torch.ones_like(Y), create_graph=True, retain_graph=True,allow_unused=True)
        dUNN = grad2UNN[0]
        dUNN2y = torch.reshape(dUNN[:, 1], shape=[-1, 1])
        diff_bd = torch.mul(Distance, dUNN2y)-U_bd
        if str.lower(loss_type) == 'l2_loss':
    ...
    def loss2bd_neumann(self, XY_bd=None, Ubd_exact=None, if_lambda2Ubd=True, loss_type='l2_loss',scale2lncosh = 0.5):
        ...
        UNN = self.DNN(XY_bd, scale=self.factor2freq, sFourier=self.sFourier)
        grad2UNN = torch.autograd.grad(UNN, XY_bd, grad_outputs=torch.ones_like(Y), create_graph=True, retain_graph=True)
        dUNN = grad2UNN[0]
        dUNN2x = torch.reshape(dUNN[:, 0], shape=[-1, 1])
        diff_bd = dUNN2x - U_bd
        if str.lower(loss_type) == 'l2_loss':
...
def solve_Multiscale_PDE(R):
    ...
    if R['PDE_type'] == '1DV':
        ...
        region_t = 0.05
        D_xy = lambda x, y: x / L
        u_bottom = lambda x, y: torch.zeros_like(y)
        u_top = lambda x, y: torch.zeros_like(y)
        u_true = lambda x, t: torch.ones_like(x) * 30
    ...
    if R['testData_model'] == 'random_generate':
        ...
        test_xy_bach_y = np.linspace(-0.05, 0.05, test_bach_size).reshape(-1, 1)
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        if R['PDE_type'] == 'Advec Heat':
            ...
            t_in_batch = dataUtilizer2torch.rand_it(batchsize_it, 1, region_a=region_b, region_b=region_t,
                                                    to_float=True, to_cuda=R['use_gpu'],
                                                    gpu_no=R['gpuNo'],use_grad2x=True)
            ...
            xl_bd_batch, xr_bd_batch, yb_bd_batch, yt_bd_batch = dataUtilizer2torch.rand_bd_2D1(
                ...
                init_l=region_b, init_r=region_t,to_float=True, to_cuda=R['use_gpu'], gpu_no=R['gpuNo'])
        ...
        if R['PDE_type'] == 'Advec Heat':
            UNN2train, loss_it = mscalednn.loss_AH(XY=xy_it_batch, D_xy=D_xy, if_lambda2Ubd=True, loss_type='l2_loss')
            loss_bd_bottom =mscalednn.loss2bd_neumann_AH(XY_bd=yb_bd_batch, Ubd_exact=u_bottom, if_lambda2Ubd=True,
                                                         loss_type='l2_loss')
            loss_bd_top =mscalednn.loss2bd_neumann_AH(XY_bd=yt_bd_batch, Ubd_exact=u_top, if_lambda2Ubd=True,
                                                      loss_type='l2_loss')
            PWB = penalty2WB * mscalednn.get_regularSum2WB()
            loss_bd = loss_bd_bottom + loss_bd_top
            # loss_it =loss_it + loss_dirich*100
        ...
        if i_epoch % 1000 == 0:
            ...
            DNN_tools.print_and_log_train_one_epoch_1(
                i_epoch, run_times, tmp_lr, PWB, loss_it.item(), loss_bd,
            ...
            if R['PDE_type'] == 'pLaplace_implicit':
                ...
                temp1 = D_xy(test_xy_torch[:, 0], test_xy_torch[:, 1])
                temp2 = mscalednn.evalue_MscaleDNN(XY_points=test_xy_torch)
                UNN2test = torch.mul(temp1, temp2) + T_in
    ...
    if True == R['use_gpu']:
        ...
        unn2test_numpy = unn2test_numpy.reshape((size2test**2, size2test**2))
    ...
    plotData.plot_Hot_solution2test(unn2test_numpy, size_vec2mat=size2test**2, actName=R['name2act_hidden'], seedNo=R['seed'], outPath=R['FolderName'])
...
if __name__ == "__main__":
    ...
    if store_file == 'Advec Heat':
        R['PDE_type'] = 'Advec Heat'
        R['equa_name'] = 'Advec Heat'
    ...
    if R['model2NN'] == 'Fourier_DNN':
        ...
        # R['hidden_layers'] = (50, 50, 50, 50, 50)
    else:
