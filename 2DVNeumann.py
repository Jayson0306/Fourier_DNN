...
class MscaleDNN(tn.Module):
    ...
    def loss2bd_neumann_x(self, XY_bd=None, T_bd=None,  Ubd_exact=None, if_lambda2Ubd=True, loss_type='l2_loss', scale2lncosh=0.5):
        assert (XY_bd is not None)
        assert (Ubd_exact is not None)
    ...
    def loss2bd_neumann_y(self, XY_bd=None, T_bd=None,  Ubd_exact=None, if_lambda2Ubd=True, loss_type='l2_loss', scale2lncosh=0.5):
        assert (XY_bd is not None)
        assert (Ubd_exact is not None)
...
def solve_Multiscale_PDE(R):
    ...
    # Ut +vx * Ux +vy *Uy - kx * Uxx -ky * Uyy = f
        ...
        u_dirich = lambda x, y, t: torch.zeros_like(x)
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        ...
        if R['PDE_type'] == '1DV':
            ...
            #边界点损失 诺依曼
            loss_bd2left = mscalednn.loss2bd_neumann_x(XY_bd=xy_left_bd, T_bd=t_bd_batch, Ubd_exact=u_left, loss_type=R['loss_type'])

            loss_bd2right = mscalednn.loss2bd_neumann_x(XY_bd=xy_right_bd, T_bd=t_bd_batch, Ubd_exact=u_right, loss_type=R['loss_type'])

            loss_bd2bottom = mscalednn.loss2bd_neumann_y(XY_bd=xy_bottom_bd, T_bd=t_bd_batch, Ubd_exact=u_bottom, loss_type=R['loss_type'])

            loss_bd2top = mscalednn.loss2bd_neumann_y(XY_bd=xy_top_bd, T_bd=t_bd_batch, Ubd_exact=u_top, loss_type=R['loss_type'])

            # 边界点 Dirichlet
            loss_bd2left1 = mscalednn.loss_bd2dirichlet(XY_bd=xy_left_bd, t_bd=t_bd_batch, Ubd_exact=u_dirich,
                                                       loss_type=R['loss_type'])

            loss_bd2right1 = mscalednn.loss_bd2dirichlet(XY_bd=xy_right_bd, t_bd=t_bd_batch, Ubd_exact=u_dirich,
                                                        loss_type=R['loss_type'])

            loss_bd2bottom1 = mscalednn.loss_bd2dirichlet(XY_bd=xy_bottom_bd, t_bd=t_bd_batch, Ubd_exact=u_dirich,
                                                         loss_type=R['loss_type'])

            loss_bd2top1 = mscalednn.loss_bd2dirichlet(XY_bd=xy_top_bd, t_bd=t_bd_batch, Ubd_exact=u_dirich,
                                                      loss_type=R['loss_type'])
            ...
            #边界损失点之和
            loss_bd_D = loss_bd2left1 + loss_bd2right1 + loss_bd2bottom1 + loss_bd2top1
            loss_bd_N = loss_bd2left + loss_bd2right + loss_bd2bottom + loss_bd2top
