...
class MscaleDNN(tn.Module):
    ...
    def loss2bd_neumann(self, XY=None, U_left=None, if_lambda2fside=True, loss_type='ritz_loss'):
        assert (XY is not None)
        assert (fside is not None)
        ...
        if str.lower(loss_type) == 'ritz_loss' or str.lower(loss_type) == 'variational_loss':
            ...
            square_loss_it = torch.sum(torch.mul(dUNN-force_side, dUNN-force_side))
            loss_it = torch.mean(square_loss_it)
        return UNN, loss_it
...
def solve_Multiscale_PDE(R):
    ...
    if R['PDE_type'] == 'general_Laplace' or R['PDE_type'] == 'Laplace':
        ...
        z_range = 2
        t_range = 100
        u_true = lambda x, t: np.zeros_like(x)
        u_left = lambda x, t: np.ones_like(x) * (p/ds)
        u_right = lambda x, t: np.zeros_like(x)
        u_bottom = lambda x, t: np.zeros_like(t)
