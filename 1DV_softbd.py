...
class MscaleDNN(tn.Module):
    ...
    def loss2bd_neumann(self, X_bd=None, T_bd=None, Ubd_exact=None,
    ...
    def loss2bd_dirichlet(self, X_bd=None, T_bd=None, Ubd_exact=None,
                        if_lambda2Ubd=True, loss_type='l2_loss', scale2lncosh=0.5):
        assert (X_bd is not None)
        assert (Ubd_exact is not None)
...
def solve_Multiscale_PDE(R):
    ...
    if R['PDE_type'] == 'PINN':
        ...
        u_init = lambda x, t: torch.zeros_like(t)
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        ...
        if R['PDE_type'] == 'PINN':
            ...
            UNN2train, loss_it = mscalednn.loss_1DV(X_in=x_in_batch, T_in= t_in_batch, loss_type=R['loss_type'], ws=ws, ds=ds)

            loss_bd2left = mscalednn.loss2bd_neumann(X_bd=xl_bd_batch, T_bd=t_bd_batch, Ubd_exact=u_left,
                                                            if_lambda2Ubd=True, loss_type=R['loss_type'])
            loss_bd2right = mscalednn.loss2bd_neumann(X_bd=xr_bd_batch, T_bd=t_bd_batch, Ubd_exact=u_right,
                                                             if_lambda2Ubd=True, loss_type=R['loss_type'])
            loss_init = mscalednn.loss2bd_dirichlet(X_bd=x_init_batch, T_bd=t_init_batch, Ubd_exact=u_init,
                                                    if_lambda2Ubd=True, loss_type=R['loss_type'])

            loss_bd = loss_bd2left + loss_bd2right
            # PWB = penalty2WB * mscalednn.get_regularSum2WB()
            loss = loss_it + loss_bd * temp_penalty_bd + loss_init * bd_penalty_init
