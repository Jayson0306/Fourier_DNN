...
def solve_Multiscale_PDE(R):
    ...
    if R['PDE_type'] == '1DV':
        if R['equa_name'] == 'eq1':
            ...
            u_left = lambda x, y, t: torch.zeros_like(x)
            u_right = lambda x, y, t: torch.zeros_like(x)
    ...
    for i_epoch in range(R['max_epoch'] + 1):
        ...
        if R['PDE_type'] == '1DV':
            ...
            UNN2train, loss_it = mscalednn.loss_it2ADVDFS(XYZ=xyz_it_batch, loss_type=R['loss_type'], kx=kx, ky=ky,
                                                          vx=vx, vy=vy, fside=f_side)

            loss_bd2left = mscalednn.loss2bd(XY_bd=xyz_left_batch, Ubd_exact=u_left, loss_type=R['loss_type'])

            loss_bd2right = mscalednn.loss2bd(XY_bd=xyz_right_batch, Ubd_exact=u_right, loss_type=R['loss_type'])

            loss_bd2bottom = mscalednn.loss2bd(XY_bd=xyz_bottom_batch, Ubd_exact=u_bottom, loss_type=R['loss_type'])

            loss_bd2top = mscalednn.loss2bd(XY_bd=xyz_top_batch, Ubd_exact=u_top, loss_type=R['loss_type'])

            loss_init = mscalednn.loss2bd(XY_bd=xyz_front_batch, Ubd_exact=u_init, loss_type=R['loss_type'])

            loss_bd = loss_bd2left + loss_bd2right + loss_init + loss_bd2bottom + loss_bd2top
